    ...
    <div className="min-h-screen bg-gray-950 text-white">
      <section className="relative overflow-hidden bg-gradient-to-br from-blue-950 via-gray-950 to-cyan-950 px-6 py-28 text-center">
        ...
        <div className="relative">
          <h1 className="mb-4 text-6xl font-black tracking-tight lg:text-8xl">${v.brandName}</h1>
          <p className="mx-auto mb-8 max-w-xl text-lg text-blue-200/70">${v.tagline}</p>
      ...
      <section className="mx-auto max-w-7xl px-6 py-16">
        ...
        <p className="mb-10 text-center text-sm text-gray-500">Top-rated ${v.niche} products</p>
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white">
      <section className="border-b border-gray-800 px-6 py-12 text-center">
        <h1 className="text-4xl font-black tracking-tight">${v.brandName} Store</h1>
        <p className="mt-2 text-sm text-gray-500">All ${v.niche} products</p>
      </section>
      <section className="mx-auto max-w-7xl px-6 py-12">
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {products.map((p: any) => (
            <a key={p.id} href={\`/product/\${p.handle}\`} className="group rounded-xl border border-gray-800 bg-gray-900/50 p-4 transition hover:border-blue-500">
              {p.thumbnail && (
                <div className="mb-4 aspect-square overflow-hidden rounded-lg bg-gray-800">
                  <img src={p.thumbnail} alt={p.title} className="h-full w-full object-cover transition group-hover:scale-105" loading="lazy" />
                </div>
...
export const techTemplate: SiteTemplate = {
  id: 'tech',
  name: 'Tech / Gadgets',
