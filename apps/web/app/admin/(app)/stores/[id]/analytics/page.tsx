  ...
  return (
    <div className="space-y-8">
      ...
      <header className="flex flex-wrap items-end justify-between gap-4">
        ...
        <div className="flex items-center gap-1 border border-zinc-200 rounded-full p-1 bg-white">
      ...
      <section className="border border-zinc-200 rounded-2xl overflow-hidden bg-white">
              ...
              return (
                <div key={name}>
                  <div className="flex items-baseline justify-between gap-4 mb-1.5">
                    <span className="text-sm font-medium text-zinc-800">{FUNNEL_LABEL[name]}</span>
        ...
        {store.clarity_id && (
          <div className="px-6 py-3 border-t border-zinc-200/60 bg-zinc-50/60 text-xs text-zinc-500">
...
function ConnState({ label, set }: { label: string; set: boolean }) {
  return (
    <div className="flex items-center gap-2">
      <span
        className={`inline-block h-1.5 w-1.5 rounded-full ${set ? 'bg-emerald-500' : 'bg-zinc-300'}`}
        aria-hidden="true"
      />
